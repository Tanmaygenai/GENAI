...
public class ApprovalDocumentServiceImpl implements ApprovalDocumentService {
	...
	public String uploadApprovalConfigDocument(byte[] configFileBytes) {
	...
	public String checkValidFile(String filePath, String outputDirectory) {
        ...
        try (Workbook workbook = new XSSFWorkbook(filePath)) {
            for (int sheetIndex = 0; sheetIndex < workbook.getNumberOfSheets(); sheetIndex++) {
            	...
            	if(!sheet.getSheetName().equals("LiabilityLimit") && !sheet.getSheetName().equals("MedicalPayments") 
            		    && !sheet.getSheetName().equals("CollDeductible") && !sheet.getSheetName().equals("CompDeductible")) {
	                 c = c + 1;
	                 break;
            	}
            }
	...
	public String getApprovalConfigDocument() {
		String message = "";
		try {
			String awsSecretAccessKeyValue = cryptoUtil.decryptResult(awsSecretAccessKey);
			String awsAccessKeyIdValue = cryptoUtil.decryptResult(awsAccessKeyId);
	...
	public ResponseEntity<Resource> downloadApprovalConfigDocument() {
        try {
        	String awsSecretAccessKeyValue = cryptoUtil.decryptResult(awsSecretAccessKey);
        	String awsAccessKeyIdValue = cryptoUtil.decryptResult(awsAccessKeyId);
        	AwsBasicCredentials awsCreds = AwsBasicCredentials.create(awsAccessKeyIdValue, awsSecretAccessKeyValue);
