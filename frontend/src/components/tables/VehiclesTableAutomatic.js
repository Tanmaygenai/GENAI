...
const VehiclesTableAutomatic = ({ vehicles,modalState, setModalState, openModal }) => {
    const deleteModalData= (ind)=>{
        const clonePol = [...modalState.VehicleData]
        clonePol.splice(ind,1)
        setModalState({ ...modalState, VehicleData: clonePol, addPolicyTitle: "Add Vehicle" })
        
    }
    const changeModalData= async(ind,val)=>{
        await setModalState({...modalState, editIndex:ind})
       await openModal('mainModal','Add Vehicle',ind)
    }
...
export default VehiclesTableAutomatic;