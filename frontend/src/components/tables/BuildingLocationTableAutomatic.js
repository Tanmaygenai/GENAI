...
const BuildingLocationTableAutomatic = ({ buildingLoc, modalState, setModalState, openModal }) => {
    const deleteModalData= (ind)=>{
        const clonePol = [...modalState.BuildingLocData]
        clonePol.splice(ind,1)
        setModalState({ ...modalState, BuildingLocData: clonePol, addPolicyTitle: "Add Building" })
    }
    const changeModalData= async(ind,val)=>{
        await setModalState({...modalState, editIndex:ind})
        await openModal('mainModal','Add Building',ind)
    }
...
export default BuildingLocationTableAutomatic;