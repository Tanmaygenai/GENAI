...
const LocationsTableAutomatic = ({ locations, modalState, setModalState, openModal }) => {

    const deleteModalData= (ind)=>{
        const clonePol = [...modalState.LocationData]
        clonePol.splice(ind,1)
        setModalState({ ...modalState, LocationData: clonePol, addPolicyTitle: "Add New Location" })
    }
    const changeModalData= async(ind,val)=>{
        await setModalState({...modalState, editIndex:ind})
        await openModal('mainModal','Add New Location',ind)
    }
...
export default LocationsTableAutomatic;