...
import VehicleDetailsExcel from '../../dummydata/VehicleDetailsexcel.xlsx';


const VehicleDetails= ({modalState, openModal, setValue, getValues, setModalState,downloadTriggered,setDownloadTriggered}) => {
    ...
    const handleUpload = async (data) => {
        ...
        try {
            ...
            console.log("in driver details",modalState.VehicleData);
            setModalState((prevState) => ({
        ...prevState,
        VehicleData: [...prevState.VehicleData, ...response.data],
      }));
  ...
  const handleDownload = () => {
    ...
    console.log("file",VehicleDetailsExcel);

    fetch(VehicleDetailsExcel)
    .then((response) => {
    ...
    .then((blob) => {
      const url = URL.createObjectURL(blob);
        ...
        link.href = url;
        link.download = "VehicleDetails.xlsx";
        link.click();
        setLoader(false);
        setDownloadTriggered(false);
      })
    ...
    <div className="grid2">
     <MultipleFile multipleImages={multipleImages} setMultipleImages={setMultipleImages} register={register} errors={errors}/>
        <div className="form-element-left">
            <button type="submit" onClick={handleSubmit(handleUpload)} className="btn blue" >Upload Vehicles</button>
        </div>
    </div>
  );
};
